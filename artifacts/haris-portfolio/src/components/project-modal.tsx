      ...
      <div className="relative flex max-h-[calc(100dvh-56px)] sm:max-h-[calc(100dvh-64px)] w-full max-w-[680px] flex-col overflow-hidden rounded-sm border-2 border-black bg-[#f0f0f0] text-black shadow-[6px_6px_0px_#000000] mac-window-frame animate-in fade-in zoom-in-95 duration-200 my-auto">
        ...
        <div className="flex h-8 items-center justify-between border-b-2 border-black bg-[#e5e5e5] px-2 sm:px-2.5">
          <button
            ...
            className="flex h-6 w-6 sm:h-4 sm:w-4 items-center justify-center border-2 border-black bg-white shadow-[1px_1px_0px_#000000] hover:bg-black hover:text-white cursor-pointer shrink-0"
        ...
        <div className="flex flex-wrap items-center justify-between gap-1 border-b-2 border-black bg-[#f8f8f8] px-2.5 sm:px-4 py-1 sm:py-2 text-xs">
          <div className="flex items-center gap-1.5 sm:gap-2">
            <span className="rounded bg-black px-1.5 sm:px-2 py-0.5 font-mono text-[9px] sm:text-[10px] font-bold text-[#d8ee57]">
              SYSTEM DIALOG 7.5
            </span>
            <span className="font-mono text-[9.5px] sm:text-[11px] font-bold text-black truncate max-w-[170px] sm:max-w-none">
              Target: hariskumaar.9@gmail.com
            </span>
          </div>
          <span className="font-mono text-[9px] sm:text-[10px] text-black/60 hidden sm:inline">256-Bit SSL · Verified EmailJS Gateway</span>
        </div>

        {/* Modal Body */}
        <div className="overflow-y-auto p-3.5 sm:p-7 mac-scrollbar bg-white overscroll-contain">
          <div className="mb-4 sm:mb-5 border-b-2 border-black/20 pb-2.5 sm:pb-3">
            <h3 id="modal-title" className="font-mono text-lg sm:text-2xl font-bold uppercase tracking-tight text-black">
              Let's Discuss Your <span className="underline decoration-[#e97857] decoration-2">Next Vision.</span>
            </h3>
            <p className="mt-1 font-mono text-[11px] sm:text-xs font-medium text-[#222222]">
              Fill out this project brief. Your brief lands directly in Haris Kumaar's Gmail inbox with full project parameters.
            </p>
          </div>

          <form onSubmit={handleSubmit} className="space-y-3.5 sm:space-y-4" data-testid="form-project-modal">
            {/* Contact details */}
            <div className="grid gap-2.5 sm:gap-3 sm:grid-cols-2">
              <div>
                <label htmlFor="modal-name" className="mb-1 block font-mono text-[10px] sm:text-[11px] font-bold uppercase text-black">
                  Your Name *
                </label>
                <input
                  id="modal-name"
                  ...
                  className="w-full rounded border-2 border-black bg-white px-2.5 sm:px-3 py-1.5 sm:py-2 font-mono text-xs text-black font-medium placeholder:text-neutral-500 focus:border-black focus:outline-none focus:ring-2 focus:ring-black"
                  data-testid="input-modal-name"
                />
              </div>
              <div>
                <label htmlFor="modal-email" className="mb-1 block font-mono text-[10px] sm:text-[11px] font-bold uppercase text-black">
                  Email Address *
                </label>
                <input
                  id="modal-email"
            ...
            <div>
              <label className="mb-1 flex items-center gap-1.5 font-mono text-[10px] sm:text-[11px] font-bold uppercase text-black">
                <Layers size={12} /> Project Scope / Focus Area
              </label>
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-1.5 sm:gap-2">
                {PROJECT_TYPES.map((type) => (
                  <button
                    ...
                    className={`mac-button rounded border-2 border-black px-2 sm:px-2.5 py-1.5 sm:py-2 text-left font-mono text-[10px] sm:text-[11px] font-bold transition-all cursor-pointer ${
                      projectType === type
