...
import { CaseStudyModal, caseStudiesData, type CaseStudy } from '@/components/case-study-modal';
import {
  MacMenuBar,
  DesktopIcon,
  MacWindow,
...
import { kanyeStrongerPlayer } from '@/lib/kanye-stronger-8bit';
import {
  AlertCircle,
  ArrowDown,
  ArrowRight,
...
const projects: Project[] = [
  {
    ...
    title: 'Vamvora Technologies — Digital Transformation',
    description:
      'Complete UX research, user mapping, and enterprise website redesign. Restructured dense cloud and cybersecurity offerings into frictionless B2B buyer journeys.',
  ...
  },
  {
    ...
    description:
      'Enterprise learning management platform designed to simplify certification learning through structured dashboards, personalized learning paths, and progress-driven UX.',
  ...
  },
  {
    ...
    description:
      'Renewable energy platform designed to educate users about solar adoption while simplifying access to residential and commercial solar solutions.',
  ...
  },
  {
    ...
    description:
      'A luxury interior experience website crafted to showcase premium residential spaces through immersive storytelling, editorial-inspired layouts, and cinematic presentation.',
  ...
  },
  {
    ...
    description:
      'AI-powered pharmacy and healthcare ecosystem designed to simplify digital medicine purchases, improve accessibility, and increase mobile conversion rates by 40%.',
...
function Home() {
  ...
  const openCaseStudy = (studyId: string) => {
    const found = caseStudiesData.find((s) => s.id === studyId);
    if (found) {
  ...
  useEffect(() => {
    const unsub = kanyeStrongerPlayer.subscribe((playing) => {
    ...
    <div className={`fixed inset-0 h-screen w-screen overflow-hidden select-none font-mono ${dark ? 'dark' : ''}`}>
      ...
      >
        ...
        <MacMenuBar
          ...
          onToggleBgm={() => kanyeStrongerPlayer.toggle()}
        />
        ...
        {/* Case Study Detailed View Modal - INSIDE CRT SCREEN */}
        <CaseStudyModal
          study={selectedStudy}
          isOpen={isStudyModalOpen}
          onClose={() => setIsStudyModalOpen(false)}
          onOpenInquire={(studyId) => {
              ...
              <div className="flex flex-wrap items-center justify-between gap-2">
                <div>
                  ...
                  <p className="font-mono text-xs font-medium text-[#222222]">
                    Useful systems, beautifully considered. Click "Case Study" on any card for dedicated research &amp; live UI details.
                  </p>
                </div>
                <button
                  type="button"
              ...
              {projects.map((project) => (
                ...
                >
                  <div>
                    ...
                    {/* Quick Direct Links if present (e.g. Vamvora live & Notion research) */}
                    {(project.liveUrl || project.userResearchUrl) && (
                      <div className="mt-3 flex flex-wrap gap-2 pt-2 border-t border-dashed border-black/20">
                        {project.liveUrl && (
                          <a
                            href={project.liveUrl}
                            target="_blank"
                            rel="noopener noreferrer"
                            className="inline-flex items-center gap-1 text-[10px] font-bold text-black underline hover:text-emerald-700"
                          >
                            <Globe size={11} /> Live Website <ExternalLink size={9} />
                          </a>
                        )}
                        {project.userResearchUrl && (
                          <a
                            href={project.userResearchUrl}
                            target="_blank"
                            rel="noopener noreferrer"
                            className="inline-flex items-center gap-1 text-[10px] font-bold text-black underline hover:text-blue-700"
                          >
                            <FileText size={11} /> Notion Research <ExternalLink size={9} />
                          </a>
                        )}
                      </div>
                    )}
                  ...
                  <div className="mt-5 pt-3 border-t border-black/20 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                    ...
                    <div className="flex items-center gap-1.5 flex-wrap self-start sm:self-auto shrink-0">
                      <button
                        ...
                        className="mac-button inline-flex items-center gap-1 rounded border border-black bg-[#d8ee57] px-2.5 py-1 font-mono text-[10px] font-bold text-black hover:bg-[#cbe348] cursor-pointer"
                      >
                        <BookOpen size={11} /> Case Study <ArrowUpRight size={10} />
                      </button>
                      <button
                        type="button"
                        onClick={() => openWindow('project-modal')}
                        className="mac-button inline-flex items-center gap-1 rounded border border-black bg-[#ececec] px-2.5 py-1 font-mono text-[10px] font-bold text-black hover:bg-black hover:text-white cursor-pointer"
                      >
        ...
        <div className="absolute bottom-2 sm:bottom-2.5 left-1/2 -translate-x-1/2 z-40 flex max-w-[96%] items-center gap-1 sm:gap-2 rounded-full border-2 border-black bg-white/95 px-2 sm:px-3.5 py-1 sm:py-1.5 shadow-[4px_4px_0px_#000000] backdrop-blur-md overflow-x-auto no-scrollbar">
          ...
          <button
            ...
            className="mac-button flex items-center gap-1 rounded-sm bg-black border border-black px-2 sm:px-2.5 py-1 font-mono text-[10px] sm:text-[11px] font-bold text-[#d8ee57] hover:bg-[#d8ee57] hover:text-black cursor-pointer shrink-0 transition-colors"
          >
